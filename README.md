...
> **前端开发者注意**：`start.bat` 读的是 `static/` 构建产物。修改前端源码后需 `cd frontend && npm run build` 重新构建，然后重启服务。开发调试推荐用 `npm run dev`（热更新）。
...
```bash
# 前端开发（热更新，改代码实时生效，需同时启动 python server.py）
cd frontend
npm install
npm run dev
# 访问 http://localhost:5173

# 前端构建（生成 static/，供 start.bat / python server.py 使用）
cd frontend
npm run build

# 生产模式启动（读 static/ 构建产物，不会热更新）
start.bat          # Windows
./start.sh         # Mac/Linux
...
```
| 启动方式 | 端口 | 读取内容 | 改代码是否实时生效 |
|---------|------|---------|------------------|
| `npm run dev` | 5173 | 前端源码 | ✅ 热更新 |
| `start.bat` / `python server.py` | 8900 | `static/` 构建产物 | ❌ 需重新 `npm run build` |
